...
import { startLoginSession, finishLoginSession, sessionScreenshot, sessionControl, testWork, executeWork } from './lib/executor.js';
...
const secret=process.env.AUTH_SECRET || 'rh-work-cloud-v1-change-this-secret';
...
app.post('/api/browser/start',guard,async(req,res)=>{try{res.json(await startLoginSession())}catch(e){res.status(400).json({error:e.message})}});
app.post('/api/browser/finish',guard,async(req,res)=>{try{res.json(await finishLoginSession())}catch(e){res.status(400).json({error:e.message})}});
app.get('/api/browser/screenshot',guard,async(req,res)=>{try{res.json(await sessionScreenshot())}catch(e){res.status(400).json({error:e.message})}});
app.post('/api/browser/control',guard,async(req,res)=>{try{const a=req.body||{}; const allowed=['click_mouse','type_text','press_key','scroll','wait','take_screenshot']; if(!allowed.includes(a.action))throw new Error('INVALID_ACTION');res.json(await sessionControl(a))}catch(e){res.status(400).json({error:e.message})}});
app.post('/api/work/test',guard,async(req,res)=>{try{res.json(await testWork())}catch(e){res.status(400).json({error:e.message})}});
app.post('/api/work/run',guard,async(req,res)=>{try{res.json(await executeWork(req.body?.reason||'manual'))}catch(e){res.status(400).json({error:e.message})}});
...
async function tick(){
  ...
  try{
    ...
    if(s.settings.autoContinue && s.runtime.status==='waiting' && s.runtime.resetAt && new Date(s.runtime.resetAt).getTime()<=Date.now()){
      ...
      await executeWork('limit-recovery').catch(()=>{});
    }
    ...
    if(sc.enabled && sc.time){
      ...
      if(due && s.runtime.lastScheduleKey!==key){updateState({runtime:{lastScheduleKey:key}});addLog('schedule_due',`定时任务触发 ${key}`);await executeWork('schedule').catch(()=>{});}
    }
...
app.listen(port,'0.0.0.0',()=>console.log(`RH Work Cloud listening on ${port}`));
