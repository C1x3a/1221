...
public final class FillSession {
    ...
    public FillSession(int index,boolean pending,boolean verificationStarted){this.index=index;this.pending=pending;this.verificationStarted=pending&&verificationStarted;}
    public boolean maySubmit(long now){return !pending&&(!retryable||now-sentAt>=1200);}
    public void submitted(long now){pending=true;retryable=false;successSignal=false;verificationStarted=false;attempts++;sentAt=now;}
    public void startedVerification(){if(pending)verificationStarted=true;}
