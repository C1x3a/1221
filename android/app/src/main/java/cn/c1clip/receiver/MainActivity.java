...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle saved){super.onCreate(saved);getWindow().setStatusBarColor(Color.WHITE);getWindow().setNavigationBarColor(Color.WHITE);getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR|View.SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR);
        ...
        label(body,"多机接收",28,ink,true);label(body,"0.5  ·  接收、整理、稳定填写",13,muted,false);
        ...
        stopTask=action(card,"停止填写",false,()->FillService.cancel("已停止，进度已保留"));stopTask.setVisibility(View.GONE);
    ...
    private void showSettings(){
        ...
        action(panel,"开启辅助填写权限",false,()->new AlertDialog.Builder(this).setTitle("辅助填写权限").setMessage("点击猫眼或票星球后，服务会识别当前页面，从首页、我的、人员列表或填写页继续。每次点击后会等待页面变化，不会连续返回。悬浮小窗可查看进度、暂停、继续或切换平台；如遇验证码、登录失效或资料校验错误，需要你先处理。页面内容不会上传。").setPositiveButton("前往设置",(d,w)->safeStart(new Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS))).setNegativeButton("取消",null).show());
        ...
        label(panel,"无法识别页面时只会尝试重新打开一次、单次返回一次，随后暂停，不会循环返回。回到本 APP 再点击平台可重新启动。",12,muted,false);
    ...
    private void launch(String label,String pkg,boolean fill){try{
        ...
        if(fill){JSONObject latest=Vault.latest(this);if(latest==null||batch==null||!latest.getString("id").equals(batch.getString("id"))){refresh();toast("资料已更新，请重新选择人员");return;}String after=getPreferences(0).getBoolean("afterEnabled",false)?getPreferences(0).getString("afterPkg",""):"";if(getPreferences(0).getBoolean("afterEnabled",false)&&after.isEmpty()){toast("请先选择完成后切换的应用，或关闭该功能");return;}if(FillService.isRunning())FillService.cancel("正在重新启动填写");FlowRules.Platform platform=label.equals("票星球")?FlowRules.Platform.PIAOXINGQIU:FlowRules.Platform.MAOYAN;int stay=getPreferences(0).getInt("afterDelayChoice",1)==0?5000:10000;String maoyan=getPreferences(0).getString("app_猫眼","");String planet=getPreferences(0).getString("app_票星球","");if(!FillService.start(pkg,platform,people,batch.getString("id"),after,stay,maoyan,planet)){toast("辅助服务未启用");return;}}
