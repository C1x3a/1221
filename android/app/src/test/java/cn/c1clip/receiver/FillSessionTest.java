...
public class FillSessionTest {
    ...
    @Test public void retriesRequireExplicitFailureAndAreBounded(){FillSession s=new FillSession(0,false);for(int i=0;i<6;i++){long now=(i+1)*2000;assertTrue(s.maySubmit(now));s.submitted(now);assertFalse(s.maySubmit(now+1500));s.failedExplicitly();assertFalse(s.maySubmit(now+100));}assertFalse(s.maySubmit(50000));assertEquals(0,s.index);}
