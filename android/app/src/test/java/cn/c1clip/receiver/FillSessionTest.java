...
public class FillSessionTest {
    @Test public void saveAttemptsAreStrictlyBounded(){assertEquals(3,FillSession.MAX_ATTEMPTS);}
    ...
    @Test public void verificationBackIsPersistableAndClearedOnlyByOutcome(){FillSession s=new FillSession(1,false);s.submitted(100);s.startedVerification();assertTrue(s.verificationStarted);FillSession resumed=new FillSession(s.index,s.pending,s.verificationStarted);assertTrue(resumed.verificationStarted);resumed.failedExplicitly();assertFalse(resumed.verificationStarted);}
