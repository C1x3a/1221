...
export function parseLines(text){const lines=String(text).split(/\r?\n/).filter(s=>s.trim().length);if(!lines.length)throw new Error('请至少填写一条信息');if(lines.length>LIMIT)throw new Error('每次最多发送 20 条独立信息');if(lines.some(s=>s.length>4000)||lines.join('').length>MAX_CHARS)throw new Error('内容过长，请分批发送');return lines}
...
export function readPair(input){let raw=String(input).trim();if(raw.includes('#pair='))raw=raw.split('#pair=')[1];raw=raw.split(/[\s&]/)[0];if(raw.length>2000)throw new Error('连接信息过长');let p;try{p=JSON.parse(new TextDecoder().decode(from64(raw)))}catch{throw new Error('连接链接不完整，请重新复制或扫码')};if(p.v!==1||!/^c1clip-[A-Za-z0-9_-]{20,50}$/.test(p.host)||!/^d[A-Za-z0-9_-]{10,40}$/.test(p.device)||typeof p.name!=='string'||p.name.length>60||typeof p.key!=='string'||from64(p.key).length!==32)throw new Error('连接信息格式不正确');return p}
const keys=new Map();
